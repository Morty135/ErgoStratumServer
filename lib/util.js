...
exports.addressFromEx = function(exAddress, ripdm160Key){
    try {
        ...
        var addrBase = Buffer.concat([versionByte, new Buffer.alloc(ripdm160Key, 'hex')]);
        var checksum = exports.sha256d(addrBase).subarray(0, 4);
...
exports.reverseHex = function(hex){
    return exports.reverseBuffer(new Buffer.alloc(hex, 'hex')).toString('hex');
};
...
exports.uint256BufferFromHash = function(hex){
    ...
    var fromHex = new Buffer.alloc(hex, 'hex');

    if (fromHex.length != 32){
        var empty = new Buffe.alloc(32);
        empty.fill(0);
        fromHex.copy(empty);
        fromHex = empty;
    }
...
exports.varStringBuffer = function(string){
    var strBuff = new Buffer.alloc(string);
...
exports.serializeNumber = function(n){
    ...
    if (n >= 1 && n <= 16) return new Buffer.alloc([0x50 + n]);
    var l = 1;
    var buff = new Buffer.alloc(9);
    while (n > 0x7f)
    {
...
exports.pubkeyToScript = function(key){
    ...
    pubkey[0] = 0x21;
    pubkey[34] = 0xac;
    new Buffer.alloc(key, 'hex').copy(pubkey, 1);
    return pubkey;
};


exports.miningKeyToScript = function(key){
    var keyBuffer = new Buffer.alloc(key, 'hex');
    return Buffer.concat([new Buffer.alloc([0x76, 0xa9, 0x14]), keyBuffer, new Buffer.alloc([0x88, 0xac])]);
};
...
exports.addressToScript = function(addr){
    ...
    return Buffer.concat([new Buffer.alloc([0x76, 0xa9, 0x14]), pubkey, new Buffer.alloc([0x88, 0xac])]);
};
...
exports.shiftMax256Right = function(shiftRight){
    ...
    return new Buffer.alloc(octets);
};
...
exports.bufferToCompactBits = function(startingBuff){
    ...
    buff = buff.readUInt8(0) > 0x7f ? Buffer.concat([new Buffer.alloc([0x00]), buff]) : buff;

    buff = Buffer.concat([new Buffer.alloc([buff.length]), buff]);
...
exports.bignumFromBitsHex = function(bitsString){
    var bitsBuff = new Buffer.alloc(bitsString, 'hex');
